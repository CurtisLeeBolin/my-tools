...
#  av-web.py
#
#  Copyright 2016 Curtis Lee Bolin <CurtisLeeBolin@gmail.com>
#
    ...
    for f in fileList:
        if os.path.isfile(f):
            ...
            if checkFileType(fileExtension):
                ...
                outputFile = '{}/{}.mp4'.format(dirDict['output'], fileName)
                outputFilePart = '{}.part'.format(outputFile)
                os.rename(f, inputFile)
                printLog('Starting: {}'.format(fileName))
                if inputArgs.mono:
                    args = shlex.split('ffmpeg -i {} -map_metadata -1 -c:v libx264 -profile:v high -ac 1 -strict experimental -c:a aac -q:a 3 -movflags faststart -metadata title="{}" -y -f mp4 {}'.format(inputFile.__repr__(), fileName, outputFilePart.__repr__()))
                else:
                    args = shlex.split('ffmpeg -i {} -map_metadata -1 -c:v libx264 -profile:v high -ac 2 -strict experimental -c:a aac -q:a 3 -movflags faststart -metadata title="{}" -y -f mp4 {}'.format(inputFile.__repr__(), fileName, outputFilePart.__repr__()))
