    ...
    for f in fileList:
        if os.path.isfile(f):
            ...
            if checkFileType(fileExtension):
                ...
                outputFile = '{}/{}.mp3'.format(dirDict['output'], fileName)
                outputFilePart = '{}.part'.format(outputFile)
                os.rename(f, inputFile)
                printLog('Starting: {}'.format(fileName))
                if inputArgs.mono:
                    args = shlex.split('ffmpeg -i {} -vn -ac 1 -c:a mp3 -q:a 3 -sn -metadata title="{}" -y -f mp3 {}'.format(inputFile.__repr__(), fileName, outputFilePart.__repr__()))
                else:
                    args = shlex.split('ffmpeg -i {} -vn -c:a mp3 -q:a 3 -sn -metadata title="{}" -y -f mp3 {}'.format(inputFile.__repr__(), fileName, outputFilePart.__repr__()))
